...
public class ChallengeService {
            ...
            if (challenge.getBadge() != null) {
                try {
                    ...
                    log.info("Awarded badge {} to student {} for completing challenge {}", 
                            challenge.getBadge().getId(), studentId, challengeId);
                } catch (Exception e) {
    ...
    public List<ChallengeParticipation> getMyParticipations() {
        ...
        for (ChallengeParticipation participation : participations) {
            try {
                // Load student (to avoid lazy loading)
                ...
                try {
                    ...
                    if (challenge != null) {
                        ...
                        if (challengeRepository.existsById(challengeId)) {
                            ...
                            // Load badge if exists (badge is EAGER, but ensure it's loaded)
                            if (challenge.getBadge() != null) {
                                challenge.getBadge().getId();
                                challenge.getBadge().getName();
                                challenge.getBadge().getDescription();
                                challenge.getBadge().getIconUrl();
                            }
