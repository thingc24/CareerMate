# PowerShell script to setup User-Service database
...
Write-Host "User-Service Database Setup" -ForegroundColor Cyan
Write-Host "=========================================" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    if ($migrate -eq "y" -or $migrate -eq "Y") {
        ...
        # Export data
        $EXPORT_FILE = Join-Path $SCRIPT_DIR "user_service_data.sql"
        Write-Host "Exporting data to $EXPORT_FILE..." -ForegroundColor Yellow
        & $PG_DUMP_PATH -U $DB_USER -d $OLD_DB -t "$SCHEMA.*" --data-only --column-inserts -f $EXPORT_FILE
    ...
    Write-Host "2. Restart user-service" -ForegroundColor White
    Write-Host ""
