    ...
    <div className="max-w-6xl mx-auto px-4 py-4">
      {!roadmap ? (
        <div className="bg-white dark:bg-gray-900 rounded-lg shadow-sm border border-gray-200 dark:border-gray-800 p-8">
          <form onSubmit={handleGenerate} className="space-y-6">
            ...
            <div>
              ...
              <input
                type="text"
                className="w-full px-4 py-2 border border-gray-300 dark:border-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 mb-2 dark:bg-gray-800 dark:text-white"
        ...
        <div className="space-y-6">
          {/* Roadmap Overview */}
          <div className="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-gray-800 dark:to-gray-800 rounded-xl shadow-lg p-8 border border-blue-200 dark:border-gray-700">
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-4">Lộ trình của bạn</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              <div>
                <p className="text-sm text-gray-600 dark:text-gray-300 mb-1">Vị trí mục tiêu</p>
                <p className="text-lg font-semibold text-gray-900 dark:text-white">{formData.targetRole}</p>
              </div>
              <div>
                <p className="text-sm text-gray-600 dark:text-gray-300 mb-1">Kỹ năng hiện tại</p>
                <p className="text-lg font-semibold text-gray-900 dark:text-white">{formData.currentSkills}</p>
              </div>
            </div>
          </div>
          ...
          {roadmap.summary && (
            <div className="bg-white dark:bg-gray-900 rounded-lg shadow-sm border border-gray-200 dark:border-gray-800 p-6 mb-6">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">
                <i className="fas fa-info-circle mr-2 text-blue-600 dark:text-blue-400"></i>Tóm tắt lộ trình
              </h3>
              <p className="text-gray-700 dark:text-gray-300">{roadmap.summary}</p>
              {roadmap.timeline && (
          ...
          {roadmap.steps && roadmap.steps.length > 0 ? (
            <div className="space-y-6">
              ...
              {roadmap.steps.map((step, idx) => (
                <div
                  key={idx}
                  className="bg-white dark:bg-gray-900 rounded-xl shadow-md border border-gray-200 dark:border-gray-800 p-6 relative hover:shadow-lg transition-shadow"
                >
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      {step.skills && step.skills.length > 0 && (
                        <div className="mb-4">
                          <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">
                            <i className="fas fa-code mr-1 text-blue-600 dark:text-blue-400"></i>Kỹ năng cần học:
                          </h4>
                          <div className="flex flex-wrap gap-2">
                            {step.skills.map((skill, skillIdx) => (
                              <span
                                key={skillIdx}
                                className="px-3 py-1 bg-blue-50 dark:bg-gray-800 text-blue-700 dark:text-blue-300 text-sm rounded-full border border-blue-200 dark:border-gray-700 font-medium"
                              >
                                {skill}
                              </span>
                            ))}
                      ...
                      {step.resources && step.resources.length > 0 && (
                        <div className="mb-4">
                          <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">
                            <i className="fas fa-book mr-1 text-green-600 dark:text-green-400"></i>Tài nguyên học tập:
                          </h4>
                      ...
                      {step.projects && step.projects.length > 0 && (
                        <div className="mb-4">
                          <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">
                            <i className="fas fa-project-diagram mr-1 text-purple-600 dark:text-purple-400"></i>Đề xuất project:
                          </h4>
                          <ul className="list-disc list-inside space-y-1 text-gray-600 dark:text-gray-300 text-sm">
                      ...
                      {step.milestones && step.milestones.length > 0 && (
                        <div className="mb-4">
                          <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">
                            <i className="fas fa-flag-checkered mr-1 text-orange-600 dark:text-orange-400"></i>Mốc quan trọng:
                          </h4>
                          <ul className="list-disc list-inside space-y-1 text-gray-600 dark:text-gray-300 text-sm">
          ...
          {roadmap.skillsGap && roadmap.skillsGap.length > 0 && (
            <div className="bg-yellow-50 dark:bg-gray-800 rounded-lg shadow-sm border border-yellow-200 dark:border-gray-700 p-6 mt-6">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-3">
                <i className="fas fa-exclamation-triangle mr-2 text-yellow-600 dark:text-yellow-400"></i>Kỹ năng còn thiếu
              </h3>
              <div className="flex flex-wrap gap-2">
                {roadmap.skillsGap.map((skill, idx) => (
                  <span
                    key={idx}
                    className="px-3 py-1 bg-yellow-100 dark:bg-gray-700 text-yellow-800 dark:text-yellow-300 text-sm rounded-full border border-yellow-300 dark:border-gray-600"
                  >
                    {skill}
                  </span>
                ))}
          ...
          {roadmap.recommendedCourses && roadmap.recommendedCourses.length > 0 && (
            <div className="bg-green-50 dark:bg-gray-800 rounded-lg shadow-sm border border-green-200 dark:border-gray-700 p-6 mt-6">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-3">
                <i className="fas fa-graduation-cap mr-2 text-green-600 dark:text-green-400"></i>Khóa học đề xuất
              </h3>
              <div className="space-y-2">
                {roadmap.recommendedCourses.map((course, idx) => (
                  <div key={idx} className="flex items-start gap-2">
                    ...
                    <div>
                      <p className="text-gray-900 dark:text-white font-medium">{course.name || course}</p>
                      {course.platform && (
                        <p className="text-sm text-gray-600 dark:text-gray-300">Nền tảng: {course.platform}</p>
                      )}
                      {course.url && (
                        <a
                          href={course.url}
